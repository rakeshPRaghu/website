  ...
  queries: [
    {
      refId: "A",
      datasourceId: 1,
      rawSql: type === "topContributors" ? rawSql.topContributors : rawSql.newContributors,
      format: "table",
      intervalMs: 86400000,
      maxDataPoints: 1255,
    },
...
const fetchContributors = async () =>  {
  const topContributors = await request.post(settings("topContributors"), function (error, response, body) {
    if (error) {
      console.error(error);
      return;
    }
  ...
  topContributors && await request.post(settings("newContributors"), function (error, response, body) {
    if (error) {
      console.error(error);
      return;
    }
